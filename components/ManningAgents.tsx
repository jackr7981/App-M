...
import { APPROVED_AGENTS } from './agentsData';
...
export const ManningAgents: React.FC<ManningAgentsProps> = ({ userProfile }) => {
  ...
  const filteredAgents = useMemo(() => {
    ...
    return APPROVED_AGENTS.filter(agent => {
      // Search Filter
        ...
        <div className="mt-3 flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          {['All', 'Dhaka', 'Chittagong', 'Khulna'].map(city => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors border flex items-center ${cityFilter === city
